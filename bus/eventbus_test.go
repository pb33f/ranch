...
func init() {
	evtBusTest = NewEventBus().(*transportEventBus)
}
...
func TestEventBus_Boot(t *testing.T) {
	...
	assert.NotNil(t, evtBusTest.GetId())
...
func TestEventBus_ListenStream(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, "hello melody", nil))

		// send requests to make sure we're only getting requests
		// evtBusTest.SendRequestMessage(evtbusTestChannelName, 0, nil)
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 1, nil))
	}
...
func TestTransportEventBus_ListenStreamForBroadcast(t *testing.T) {
	...
	for i := 0; i < 3; i++ {
		assert.NoError(t, evtBusTest.SendBroadcastMessage(evtbusTestChannelName, "hello melody"))
...
func TestTransportEventBus_ListenStreamForDestination(t *testing.T) {
	...
	for i := 0; i < 20; i++ {
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, "hello melody", &id))

		// send requests to make sure we're only getting requests
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 0, &id))
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 1, &id))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
...
func TestEventBus_ListenOnce(t *testing.T) {
	...
	for i := 0; i < 10; i++ {
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 0, handler.GetDestinationId()))
	}

	for i := 0; i < 2; i++ {
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, 0, handler.GetDestinationId()))

		// send requests to make sure we're only getting requests
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 0, handler.GetDestinationId()))
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 1, handler.GetDestinationId()))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
...
func TestEventBus_ListenOnceForDestination(t *testing.T) {
	...
	for i := 0; i < 300; i++ {
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, 0, &dest))

		// send duplicate
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, 0, &dest))

		// send random noise
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, 0, nil))

		// send requests to make sure we're only getting requests
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 0, &dest))
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 1, &dest))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
	assert.Equal(t, 1, count)
	destroyTestChannel()
...
func TestEventBus_ListenRequestStream(t *testing.T) {
	...
	for i := 0; i < 10000; i++ {
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, "hello melody", nil))

		// send responses to make sure we're only getting requests
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, "will fail assertion if picked up", nil))
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, "will fail assertion again", nil))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
...
func TestEventBus_ListenRequestStreamForDestination(t *testing.T) {
	...
	for i := 0; i < 1000; i++ {
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, "hello melody", &id))

		// send responses to make sure we're only getting requests
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, "will fail assertion if picked up", &id))
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, "will fail assertion again", &id))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
...
func TestEventBus_ListenRequestOnce(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, "hello melody", handler.GetDestinationId()))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
	assert.Equal(t, 1, count)
	destroyTestChannel()
...
func TestEventBus_ListenRequestOnceForDestination(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, "hello melody", &dest))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
	assert.Equal(t, 1, count)
	destroyTestChannel()
...
func TestEventBus_TestErrorMessageHandling(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		...
		assert.NoError(t, evtBusTest.SendErrorMessage(evtbusTestChannelName, err, handler.GetId()))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
...
func TestEventBus_ListenFirehose(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		err := errors.New("something went wrong")
		assert.NoError(t, evtBusTest.SendErrorMessage(evtbusTestChannelName, err, nil))
		assert.NoError(t, evtBusTest.SendRequestMessage(evtbusTestChannelName, 0, nil))
		assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, 1, nil))
	}
	assert.NoError(t, evtbusTestManager.WaitForChannel(evtbusTestChannelName))
...
func TestEventBus_RequestOnce(t *testing.T) {
	...
	handler.Handle(
		func(msg *model.Message) {
			...
			assert.NoError(t, evtBusTest.SendResponseMessage(evtbusTestChannelName, "why melody is of course", msg.DestinationId))
		},
		func(err error) {})
...
func TestBifrostEventBus_AddMonitorEventListener(t *testing.T) {
	...
	bus.SendMonitorEvent(MonitorEventType("test.external.event"), "store1", nil)
	assert.Equal(t, listener1Count, 1)
	assert.Equal(t, listener2Count, 2)
