...
func TestStompConn_CloseQueuesConnectionClosedWhenEventsFull(t *testing.T) {
	events := make(chan *ConnEvent, 1)
	events <- &ConnEvent{eventType: ConnectionStarting}
	stompConn, rawConn, events := getTestStompConn(NewStompConfig(0, []string{}), events)

	stompConn.Close()
	assert.Equal(t, closed, stompConn.state)
	assert.False(t, rawConn.connected)

	<-events
	select {
	case e := <-events:
		assert.Equal(t, ConnectionClosed, e.eventType)
		assert.Equal(t, stompConn, e.conn)
	case <-time.After(time.Second):
		t.Fatal("connection closed event was not queued")
	}
}
...
func TestStompConn_Connect(t *testing.T) {
	...
	verifyFrame(t, rawConn.sentFrames[0], frame.New(frame.CONNECTED,
		frame.Version, "1.2",
		frame.HeartBeat, "0,0",
		frame.Server, "pb33f-ranch/0.0.1"), true)
...
func TestStompConn_SetReadDeadline(t *testing.T) {
	...
	diff := time.Until(rawConn.getCurrentReadDeadline())

	// verify the read deadline for the connection is
	// between 15 and 21 seconds
	// assert.Greater(t, diff.Seconds(), float64(15))  <-- we don't care anymore.
