...
func TestStompServer_SendMessageToClient(t *testing.T) {
	...
	for _, conn := range server.connectionsMap {
		stompConn := conn.(*stompConn)
		switch stompConn.rawConnection {
		case mockRwConn1:
			connMap[mockRwConn1] = stompConn
		case mockRwConn2:
			connMap[mockRwConn2] = stompConn
		}
	}
